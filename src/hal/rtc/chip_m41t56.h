...
иметь одну секунду, либо сокращенную на 128, либо увеличенную на 256 импульсов
...
Предполагая, что генератор фактически работает ровно 32 768 Гц, каждый из 31
приращений в калибровочном байте будет составлять +10,7/-5,35 секунды в
...
Первый включает в себя просто установку часов, позволяя им работать в течение
месяца и сравнение показаний с известным точным временем (например, трансляции
WWV). Хотя это может показаться грубым, это позволяет разработчику дать
